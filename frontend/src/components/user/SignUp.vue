...
export default {
  ...
  methods: {
    ...
    chkPwd () {
      const num = this.user.pwd.search(/[0-9]/g)
      const lowerEng = this.user.pwd.search(/[a-z]/ig)
      const upperEng = this.user.pwd.search(/[A-Z]/ig)
      const spe = this.user.pwd.search(/[`~!@@#$%^&*|₩₩₩'₩";:₩/?]/gi)

      if (this.user.pwd.length < 7 || this.user.pwd.length > 16) {
        this.chkPwdFeedback = undefined
        return false
      }

      if (this.user.pwd.search(/\s/) !== -1) {
        this.chkPwdFeedback = '비밀번호는 공백 없이 입력해주세요.'
      } else if (num < 0 || lowerEng < 0 || spe < 0) {
        this.chkPwdFeedback = '영문,숫자, 특수문자를 혼합하여 입력해주세요.'
      } else {
        this.chkPwdFeedback = undefined
      }
    },
